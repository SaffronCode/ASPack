...
package
{
	...
	public class UnicodeStatic
	{
		...
		public static function htmlText(target:TextField,tx:String,useCash:Boolean=true,autoSize:Boolean=false,justify:Boolean=true,splitIfToLong:Boolean=false,maxLines:uint = uint.MAX_VALUE):void
		{
			...
			else
			{
				...
				if((deactiveConvertor || !detectedArabicScript ) && !(detectArabicScript && detectedArabicScript))
				{
					...
					var tf:TextFormat;
						//target.text = tx ;
					//Get text format befor html distroy your manual format
		...
		private static function textID(yourTextField:TextField,tex:String,textMaxLine:uint=0):String
		{
			...
			var ID:String = yourTextField.width/*+','+yourTextField.height*/+','+tex.length+','+tex.substring(0,5)+','+tex.substring(tex.length-5,tex.length)+','+zipTheText(tex.substr(tex.length/2))+','+zipTheText(tex.substr(0,tex.length/2))+','+size+','+font+bold+','+textMaxLine;
			//trace("ID is : "+ID);
			return ID ;
		}
