...
package videoPlayer
{
	...
	public class myVideoPlayerBeta extends Sprite
	{
		private static var ME:myVideoPlayerBeta;
		...
		public function myVideoPlayerBeta():void
		{
			super();
			ME = this ;
...
////////////////////////////////////////////////////////playe back functions
		...
		private function enterFrame(e=null)
		{
			...
			if(seeking)
			{
				...
				persent = Math.min((seek.x-seekLine.x)/seekLine.width,1)*vid_duration;
		...
		{
			if((playIt==-1 && playBTN.currentFrame==1) || playIt==1)
			{
				...
				ns.resume();
				playBTN.gotoAndStop(2);
				dispatchEvent(new Event("EVENT_PLAYED",true,false));
			}
			else if(playBTN.currentFrame==2 || playIt==0)
			{
				ns.pause();
				playBTN.gotoAndStop(1);
				dispatchEvent(new Event("EVENT_PAUSED",true,false));
			}
		}
